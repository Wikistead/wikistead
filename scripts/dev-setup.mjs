...
// #726BUILD THE WORKSPACE PACKAGES, and do it BEFORE the store check below.
//
...
// #338a fresh `down -v` wipes the postgres volume too, so the APP database is empty. Provisioning only
...
// #338the app reads OPENFGA_MODEL_ID from .env at boot, and `tsx watch` does NOT re-read `--env-file`
