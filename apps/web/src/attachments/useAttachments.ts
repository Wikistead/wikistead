...
export function useUploadAttachment(spaceId: string | undefined, pageId: string) {
  ...
  return useMutation({
    mutationFn: async (file: File) => {
      ...
      // Read the file into memory first. Fetching a File straight from an <input>
      // as the body can abort mid-flight (the browser may detach the input's File
      // backing once the change handler returns); a detached ArrayBuffer is stable.
      const body = await file.arrayBuffer();
      // Direct cross-origin PUT to storage (server never proxies bytes). Surface
      // a clear per-file error on failure — no auto-retry (avoids pending-orphan
      // growth; the user retries manually, and the backend GC reclaims orphans).
      const put = await fetch(pres.uploadUrl, { method: "PUT", body, headers: { "Content-Type": ct } });
      if (!put.ok) throw new Error(`upload failed (${put.status})`);
      // The server confirm reads the size via HeadObject (with a short retry for
      // S3 read-after-write timing) and flips the row to confirmed.
