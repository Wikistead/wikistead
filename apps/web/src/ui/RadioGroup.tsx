    ...
    >
      {options.map((o) => (
        ...
        >
          ...
          >
            {/* #389 checked = `scale-100` — in Tailwind v4 `scale-*` and `transform-none` are
                DIFFERENT properties (`scale` vs `transform`), so the `transform-none` never overrode
                the base `scale-0` and the checked dot rendered INVISIBLE. Visibility beats the +0.4
                device-px snap chased (a sub-pixel nicety, not worth the dot).
                (original note, retained for context: the checked steady-state carries NO transform (`transform-none`, not
