    ...
    >
      {options.map((o) => (
        <RadioGroupChoice
          ...
          disabled={o.disabled}
          data-testid={optId(o.value)}
          className="group flex cursor-pointer items-start gap-2.5 rounded-md border border-border p-2.5 text-left outline-none transition-colors duration-[120ms] hover:bg-panel focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:border-accent data-[state=checked]:bg-panel"
        >
          <span
            aria-hidden
            className="mt-0.5 flex size-4 flex-none items-center justify-center rounded-full border border-input transition-colors duration-[120ms] group-data-[state=checked]:border-accent"
          >
            <span className="size-2 scale-0 rounded-full bg-accent transition-transform duration-[120ms] group-data-[state=checked]:scale-100" />
          </span>
