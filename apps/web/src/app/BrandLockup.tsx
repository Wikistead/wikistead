...
// #1169: `className` and `testId` are overridable so a second surface can render the SAME artwork at
// its own size and under its own identity. Both matter: the loading screen wants a larger mark with the
// draw animation attached, and it must NOT answer to `brand-mark` — branding.spec asserts that testid
// reaches COUNT 0 once a tenant uploads a custom logo, which a second copy on the page would falsify.
export function WikisteadMark({ className = "block h-[22px] w-[22px] flex-none", testId = "brand-mark" }: {
  className?: string;
  testId?: string;
} = {}) {
  return (
    <svg className={className} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Wikistead" data-testid={testId}>
      <path pathLength={1} d="M8 21 L24 7 L40 21 L40 35 Q40 37 38 37 L29 37 L24 43 L24 37 L10 37 Q8 37 8 35 Z" fill="none" stroke="currentColor" strokeWidth="3.5" strokeLinejoin="round" strokeLinecap="round" />
      <path pathLength={1} d="M17 24 L31 24 M17 30 L26 30" fill="none" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round" />
    </svg>
  );
}
