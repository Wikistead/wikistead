  ...
  const onToggleTask = useCallback(
    (index: number, applyFlip: () => void, checked: boolean) =>
      toggleTaskAsync({ index, applyFlip, checked }).then(() => undefined).catch((e) => {
        ...
        // #361the two 409s are told apart by their static code. `task_burst` is a TRANSIENT — a
  ...
  return (
    <AppShell sidebar={<Sidebar />} search={<SearchBox />} onLogout={logout}>
      <div style={{ display: "flex", height: "100%", minHeight: 0 }}>
        {/* #406minWidth 0 all the way down. A flex item defaults to min-width:auto, so a wide
              ...
              <div className="pointer-events-auto relative mx-auto flex w-full max-w-[740px] items-center gap-3 px-6 pt-6">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {/* #361point 3: animKey keeps the band ring animating across surface remounts too. */}
                ...
                {/* #406PageStatus stays at every width. It carries the TOC toggle AND the
...
function GuestPageContent({ minted, onBack, startEditing = false, onTitleChange }: { minted: GuestToken; onBack?: () => void; startEditing?: boolean; onTitleChange?: () => void }) {
  ...
  // module duplication). #361the serial chain is gone (it cost a round-trip per click); the
  // published reload stays COALESCED to the burst's last settle (a per-click reload repainted
  // intermediate committed states against the user's final optimistic flip — themechanism).
  ...
  const onToggleTask = useCallback(
    (index: number, applyFlip: () => void) => {
      ...
      applyFlip(); // #361no serial chain — the draft flip and the POST fire on the click frame
  ...
  return (
    ...
    <div ref={setOuterEl} className="wks-public relative h-full" style={{ fontFamily: "var(--font-body, sans-serif)" }}>
      ...
      <div className="relative h-full" style={{ minHeight: 0 }}>
        ...
        {/* #505the paginating print surface (the public CM body is virtualised too). */}
...
export function AppRoutes() {
  return (
    <Routes>
      ...
      <Route path="/watches" element={<RequireMember><Suspense fallback={<LazyFallback />}><WatchListRoute /></Suspense></RequireMember>} /> {/* #362the bell's watch list */}
