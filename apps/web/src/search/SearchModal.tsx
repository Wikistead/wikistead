...
export function SearchModal({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  const previewText = (publishedQ.data?.publishedMd ?? "").slice(0, 2000);
            ...
            <div className="hidden max-h-[60vh] flex-1 overflow-y-auto p-4 md:block" data-testid="search-preview">
              {previewId && pageQ.data ? (
                <>
                  ...
                  <div className="mt-3 text-xs leading-relaxed text-fg-dim" style={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere" }} data-testid="search-preview-body">
                    {previewText}
                  </div>
                </>
              ) : (
