...
export function Toc({
  ...
  // #345 auto-follow keeps the whole highlighted RUN (active + the light "visible" items) PLUS a couple
  ...
  // #345 Issue 3: while the pointer is OVER the overlay, hold it open (cancel the fade) so the reader can
  ...
  // #345 Issue C: draw a CONSECUTIVE run of highlighted items (visible and/or active) as ONE smooth
          ...
          <li key={h.slug}>
            <button
              ...
              className={cn(
                ...
                // #345 round only the ENDS of a highlighted run so consecutive rows merge into one shape.
                roundTop && "rounded-t",
  ...
  if (variant === "overlay") {
    ...
    return (
      <nav
        ...
        // #345 Issue 3: hovering the overlay holds it open (cancels the fade) so it can be read/scrolled.
  ...
  // #345 Issue 4: py-2 (was py-4) so the rail uses more of its vertical space for the list.
  return (
    <nav
      ref={navRef}
      aria-label={t("toc.title")}
