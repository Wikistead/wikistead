...
describe("#536one roles list, one creation flow", () => {
  ...
  it("a built-in cannot be edited, and at rest no role shows a grid at all", () => {
    ...
    // RE-AIMED AGAIN by the 2026-08-04 ruling: why does only the built-in member row get a permission
    // editing UI, when built-ins are deliberately inflexible to steer people toward paid plans? The
    // member row's control edited the tenant
