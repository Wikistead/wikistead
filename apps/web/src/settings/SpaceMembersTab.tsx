...
export function SpaceMembersTab() {
  ...
  const onAddError = (err: unknown, retry: () => void, who: string, next: string) => {
    ...
    // #607the ceiling's refusal has a reason, and a generic "something went wrong" hides it
  ...
  // #607principals whose ROLE this caller may not change (the server answers per principal).
  const frozenPrincipals = new Set(grants.filter((g) => g.changeable === false).map((g) => g.grantee));
  // #607one row per PRINCIPAL, not per capability. The owner used to appear twice — `manage`
