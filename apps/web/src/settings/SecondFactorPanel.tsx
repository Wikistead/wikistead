...
import { factorKindName, acceptedFactorKinds, browserCanUseFactorKind } from "./factor-kind"; // #653/ #673the one place a kind is a noun
...
export function SecondFactorPanel() {
  ...
  const onAddPasskey = async () => {
    ...
    // nothing — and then told its key was at fault. (#686the SHARED predicate — this was a
    ...
    <div data-testid="second-factor-panel">
      ...
      {pending ? (
        ...
        /* #653 (ruling): NO NAME FIELD before enrolling. Naming a factor is something a person
           ...
           kind (#653's fallback), and the pencil on the row is the one place a name is given.
           It also removes the last thing left standing when both add buttons are hidden (#686
          's zero-entrance panel). */
        <FormRow>
          {canAdd("totp") && (
