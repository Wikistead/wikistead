...
export function PageEmbedPicker({ open, onPick }: { open: boolean; onPick: (pageId: string | null, title?: string | null) => void }) {
  ...
  // #332 item 4 / #366 auto-highlight the FIRST REAL hit so Enter confirms immediately. cmdk with a
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) close(null); }}>
      ...
      {/* #366 FIXED modal height (centered, equal top/bottom margins) so navigating hits — which changes
      ...
      <DialogContent className="sm:max-w-5xl overflow-hidden p-0 flex flex-col h-[min(72vh,44rem)]" showCloseButton={false}>
        ...
        <Command
          ...
          onKeyDown={(e) => {
            // #366 any MANUAL list nav pins the user's selection (so a query-change auto-select can't yank it).
