...
export const atomYank: Extension = ViewPlugin.define((view) => {
  const onKeydown = (e: KeyboardEvent) => {
    // #278 point 3: a NESTED editor (slot island / editUI source pane) lives INSIDE this
    ...
    if (e.key === "y") {
      ...
      // #359 symptom 1: ADJACENT atoms chain CM's atomicRanges skip — with three touching
...
// #332 when the caret rests on a SELECTED atomSelectable atom (embed-page), the vim fat cursor's pink
...
// the focus rebuild, so the class survives without a re-pin (the timing bug). Gated to the empty-caret /
...
function blankFatCursorAt(view: EditorView): boolean {
  ...
  // #278 a fence line is macro syntax whether the widget is rendered or revealed — one rule,
