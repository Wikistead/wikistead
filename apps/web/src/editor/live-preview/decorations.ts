  ...
  toDOM(view: EditorView) {
    ...
    if (this.showsEmptyPlaceholder()) {
      ...
      if (isLayout && !view.state.readOnly) {
        ...
        //
        // #528ended that theory. Focus is single (exactly one block offers entry chrome), and
...
const livePreviewBaseTheme = EditorView.baseTheme({
  ...
  // #528this used to read `.cm-content:has(> .cm-lp-macro-raw-zone:hover)` — CSS decided, on its
  ...
  // #174 comment 1003, re-gated by #528unlike the selection pencil (drawn only when selected, so
