...
import i18n from "../../i18n"; // #455: the shared empty-macro placeholder text is localized (the #174 macros precedent)
...
// #335 on a READ-ONLY surface (Reading / template preview) footnotes AGGREGATE — the reference becomes
...
// #359 symptom 2: a NON-EMPTY (visual/mouse) selection crossing a block atom was invisible ON the
...
// #453 ONE definition of "this element is the box that IS the atom". Every root that can take
...
class CheckboxWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    if (ctl && !this.disabled) {
      ...
      // #361 suppress the NATIVE click toggle. preventDefault on mousedown does NOT stop a
...
// #525 the host renderer reports WHY it could not produce an image, because the three cases need
...
// #273 open a PDF attachment LARGE in an in-app lightbox. It reuses the EXACT same containment as the
...
class AttachmentCardWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    // (owner-approved). (supersedes the all-open mapping): affordances split by REGION
  ...
  private ensureInlineMount(view: EditorView, wrap: AtDom, size: HTMLElement) {
    ...
    void resolver.meta(this.id).then((m) => {
      ...
      void resolver.inlineUrl(this.id).then(async (url) => {
        ...
        // #273 the inline PDF is a PREVIEW — wrap it so a hover overlay reveals it opens LARGE. The
...
class TableWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    // #406 the horizontal scroll belongs to a box of its OWN, between the wrap and the table.
...
export class EditableEditUIWidget extends WidgetType {
  ...
  private mountCoEditSurface(view: EditorView, opts: HostSurfaceOptions): HostSurfaceHandle {
    ...
    // #278 the guardTeardownBlur, but scoped to the swap — a blanket guard here would break the
...
export function setTableAlign(view: EditorView, pos: number, align: FenceAlign): void {
  ...
  // #393 a GFM PIPE table has nowhere to put the attribute, so aligning one PROMOTES it to
...
function mountSlotEditIsland(view: EditorView, cell: HTMLElement, container: { from: number; to: number }, index: number, childName: "column" | "tab", dark: boolean, bodyFrom: number, caretAnchor?: number | null): boolean {
  ...
  // hand-mirrored facet list (the config-drift source diagnosed). It reaches the macro-side helper
  ...
  const onDocPointerDown = (e: PointerEvent) => {
    ...
    // gesture (supersedes the keep-editing ruling): commit here — BEFORE mousedown side effects,
    ...
    if (ownBarTab && wrapEl?.contains(ownBarTab)) {
      ...
      e.preventDefault();
      // #527 resolved LIVE, not from dataset.layoutFrom — that stamp is build-time, and this wrap
  ...
  // #278 item 1 (island lifecycle): switching tabs is DISPLAY-ONLY (class toggles + tabActiveIndex),
...
class MacroWidget extends WidgetType {
  ...
  // changes (mount / unmount the island) — never per-render churn (the eq-churn anti-test concern).
  ...
  toDOM(view: EditorView) {
    ...
    if (this.showsEmptyPlaceholder()) {
      ...
      // #278 point 2: a stable re-lookup key — the rename flow commits the island (the widget
      ...
      if (HOST_RENDERABLE.has(this.name) && this.body.trim() && renderDiagram !== noopDiagramRenderer) {
        // #525 the round trip to the operator's renderer takes ~1.4s, and until now the raw source
        ...
        void renderDiagram(this.name, this.body, this.theme).then((res) => {
          ...
          if (!("blob" in v)) {
            // #525 an INVALID diagram gets the same visible error mermaid shows; an outage says so
    ...
    if (!view.state.readOnly) {
      ...
      wrap.addEventListener("mousedown", (e) => {
        ...
        if (isLayout) {
          ...
          if (anchor != null) {
            // #527 the data-mac-pos tags (and this closure's `this.from/this.to`) are BUILD-time
      ...
      // #278 E part 1: the (add-item) renders WHENEVER the container is editable — NOT gated on
      ...
      if ((this.name === "columns" || this.name === "tabs") && !view.state.readOnly) {
        ...
        cells.forEach((cell: HTMLElement, i: number) => {
          ...
          if (this.name === "tabs") {
            cell.addEventListener("mousedown", (e) => {
              ...
              // #278 point 2: this only ever fires with NO island open in this container — an
        ...
        wrap.querySelectorAll<HTMLElement>(contentSel).forEach((slot: HTMLElement, i: number) => {
          slot.addEventListener("mousedown", (e) => {
            ...
            e.preventDefault();
            e.stopPropagation();
            // #527 the container's offsets are resolved at CLICK time, for the same reason the
...
class DetailsSummaryWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    bar.addEventListener("mousedown", (e) => {
      ...
      // #359 in WYSIWYG, ALSO park an empty caret on the block (mermaid parity) so a follow-up
...
function blockRevealed(state: EditorState, from: number, to: number): boolean {
  ...
  //
  // #359 for a non-empty selection the test is on the ANCHOR, not on containment of the whole
  ...
  // unaffected (a contained range's anchor is inside it), so the copy flow is byte-identical.
  ...
  // merely happen to be line-aligned at both ends (the j-intercept lands the head exactly on an
...
// #332 is `pos` inside a directive block that is an `atomSelectable` atom currently SELECTED (an empty
...
export interface RenderCtx {
  ...
  // #335 the document-scoped footnote aggregation, computed ONCE per build and non-null ONLY on a
...
function buildDecorations(state: EditorState, themeOverride?: MacroTheme): {
  ...
  // #335 on a read surface, append the aggregated end-of-document footnote section (numbered defs +
...
const livePreviewBaseTheme = EditorView.baseTheme({
  // #359 symptom 2: the visual-selection tint painted OVER a block atom the selection crosses
  ...
  // #413 the custom tag-suggest popup (replaces the native datalist). Popover surface tokens
  ...
  // #323 a bare `[text]` (no href) at caret-away — cancel the syntax highlighter's link tint so it reads
  ...
  ".cm-lp-table th, .cm-lp-table td": {
    ...
    height: "1.8em",
    verticalAlign: "top",
    // #406 and a min COLUMN width, for the same reason in the other axis. The editor wraps text
  ...
  // #273 the chip-pdf zoom-in cursor is retired — an inline chip always downloads (pointer).
  ...
  // the preview, and split the card into two click targets (header = download, preview =
  ...
  // #273 a DOWNLOAD card is full-surface clickable (c1660-2), so it must LOOK clickable — pointer
  ...
  // #273 (supersedes the zoom-in): the PDF INLINE card's HEADER means DOWNLOAD — pointer
  ...
  // #273 the frame lives in a relative box so the hover "open large" overlay can sit over it.
  ...
  // #273 an overlay over the inline PDF preview — invisible until the card is hovered, then it dims the
  ...
  // #273 the dim + ⤢ appear when the PREVIEW is hovered — not when anything on the card is.
  // Keyed on the whole wrap it fired from the header too, dimming the preview while the pointer was
  // over the download target: the two halves separated looked like one control again.
  ...
  // #395 UX: a soft halo AROUND the 2px ring — with tenant accents far from blue, the thin
  ...
  // #278 point 1: pointer-INERT until the chrome is actually shown. The row floats -1.5em ABOVE
  ...
  // #528 the ROW itself is owner-gated, not just the buttons inside it. It used to sit at opacity 1
  ...
  // #278 ① (the FINAL permanent form, superseding the boundary guards): every wrap-state
  ...
  // #406 a table wider than the surface SCROLLS sideways instead of being squeezed into it.
  ...
  // #406 the wrap spans the line and never scrolls (chrome anchor + resize target); the inner
  ...
  // #528 the displacement is read from a CSS VARIABLE, not written onto the element. A widget that
  ...
  // different look or trigger. The clipping that motivated the reposition is solved by the
  // island scroller's overflow: visible (below); the perma-show by parking the entry caret on
  ...
  // #278 the island hugs its content (no fixed height, ①) so it never actually scrolls
  // overflow: visible lets the -1.5em chrome (the Ctrl+↵ pill on a first-line block) float above the
  // island exactly like top-level instead of being clipped by the scroll box (the clipping that
  ...
  // #278 H: align-items STRETCH (was flex-start) so a short/EMPTY column follows the row height — its
  ...
  // #278 H: an EMPTY column / active empty tab panel shows a faint dashed accent frame on hover so the
  ...
  // #278 G: position:relative anchors the tab's × (below) absolutely so it takes NO flow width — a tab
  ...
  // #424 overflow VISIBLE (was hidden) — the box-level clip existed only for the rounded
  ...
  ".cm-lp-callout::before": {
    ...
    pointerEvents: "none",
    // #632 an absolutely-positioned child is not clipped by its box's `border-radius`, so a square
