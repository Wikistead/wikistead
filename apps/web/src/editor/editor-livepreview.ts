...
export const displayModeContent = (m: DisplayMode) =>
  m === "reading"
    // #488 no dropCursor in Reading — the mode is read-only (below), so a file cannot drop here
...
export interface LivePreviewSharedOpts {
  ...
  // nested islands via this shared opts object, so the dialog works inside columns/callouts unwired.
  openLinkPrompt?: LinkPrompt;
...
export function buildLivePreviewExtensions(opts: LivePreviewSharedOpts, env: LivePreviewLayerEnv): Extension {
  ...
  return [
    ...
    macroEdit,
    // #359 symptom 3: empty-caret copy/cut on a block atom takes the whole block's source (CM's
    // line-copy default emitted a broken first-line fragment). Shared with the islands via this factory.
    atomClipboard,
    atomSelectionTint, // #359 visual-selection tint on crossed atoms
...
export function mountLivePreview(
  ...
  opts: LivePreviewSharedOpts & { vim?: boolean; vimCompartment?: Compartment; displayMode?: DisplayMode; displayModeCompartment?: Compartment; searchPhrasesCompartment?: Compartment; onExitEdit?: () => void; onPublish?: () => void; onPublishStay?: () => void; ephemeralCollab?: EphemeralCollabFactory; macroPresence?: MacroPresence; coEditHost?: CoEditHost } = {},
): EditorView {
  ...
  const view = new EditorView({
    ...
    extensions: [
      ...
      // placement, the palette regression. It is the updateListener below; these margins stay small.)
      bandScrollMargins,
      ...
      ...(!opts.readOnly ? [floatingToolbar(), codeFenceSettingsPanel(), vimExCommands({ exitEdit: opts.onExitEdit, publish: opts.onPublish, publishStay: opts.onPublishStay })] : []), // #456 S4/ code-fence settings — panel + keyboard opener (Mod-Alt-Enter) + hover ✎, in CM's tooltip layer. #549: contextMenu moved into the shared factory (islands need it too)
