...
import { buildSearchPhrases } from "./search-phrases"; // #402localized CM search-panel phrases
...
export interface EditorProps {
  ...
  onVisibleHeadings?: (froms: number[]) => void; // #345the light-layer visible set (2-layer TOC highlight)
...
export const Editor = memo(function Editor({ docName, pageId, guestSurface = false, token, collabUrl, user, capability = "view", apiToken = "", publishedMd = null, editing = false, vim = false, displayMode = "live", onUploadImage, inlineComments, anchorGetterRef, onHeadings, onActiveHeading, onVisibleHeadings, onScrollActivity, tocJumpRef, onTaskProgress, dirtySignal, onExitEdit, onPublish, onToggleTask }: EditorProps) {
  ...
  // #402search-panel phrases Compartment — a language toggle reconfigures in place (no remount).
  ...
  // memberPageId — a guest / template preview passes none (#374pageId alone no longer implies
  ...
  useLayoutEffect(() => {
    ...
    if (surfaceKey === "view") {
      ...
      // #361per-index flip GENERATION + resync-to-published failure handling. The old revert
  ...
  // #402language toggle → reconfigure the search-panel phrases in place (vim-toggle rule:
  ...
  return (
    <div className="h-full" data-mode={surfaceKey}>
      <section className="flex h-full min-h-0 min-w-0 flex-col" data-pane="preview">
        ...
        {/* #406min-w-0 — without it this flex item refuses to shrink below its widest child, so a
