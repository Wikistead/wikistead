...
export function wireToc(
  ...
  opts: {
    ...
    // #345 the VISIBLE set (light layer) — the headings whose section intersects the viewport judgment
  ...
  // #345 jump-intent priority. After a TOC click we PIN the clicked heading as the current (dark) one
  // until the user ACTUALLY scrolls (wheel/touch/keyboard). The programmatic jump fires `scroll` events whose
  // recompute would otherwise yank the dark highlight to a neighbour on a short section (the regression).
  ...
  if (opts.onActiveHeading || opts.onVisibleHeadings || opts.onScrollActivity) {
    ...
    const compute = () => {
      raf = 0;
      // #192 / #345 two sample points (band top, ~80% down) → doc positions via posAtCoords (integer
    ...
    // #345 a REAL user scroll (wheel / touch / keyboard) releases the jump pin → normal scroll-spy resumes.
