...
//     mark is a PNG, NOT the SVG the app uses: Gmail draws nothing at all for an SVG <img>, and this
//     fallback exists precisely for the deployments that have not uploaded a logo — CE's default, dev,
//     and every unentitled tenant. An SVG here serves none of them.
//     #597: the PNG is a FAITHFUL export of the mark — same geometry, transparent, one flat palette
//     mid-tone so it reads on a white background and in a dark-mode client alike. The first version
//     invented a black tile for it, which is the thing the ruling refused: a format constraint is not
//     permission to redraw the product's mark. It is generated by scripts/render-email-mark.mjs.
import type { EmailBranding } from './outbox.js'
import type { Lang } from '../locale.js'
import { poweredBy } from './catalog.js'
...
export function renderBrandedHtml(args: {
  branding: EmailBranding; baseUrl: string; body: string; footer: string; lang: Lang;
}): string {
  ...
  const poweredByLine = args.branding.whitelabel
    ? ''
    : `<p style="font-size:11px;color:#999;margin-top:16px">${esc(poweredBy(args.lang, args.branding.productName))}</p>`;
  return [
...
export function renderBrandedText(args: { branding: EmailBranding; body: string; footer: string; lang: Lang }): string {
  const tail = args.branding.whitelabel ? '' : `\n\n${poweredBy(args.lang, args.branding.productName)}`;
