...
// #342a readable dark for PlantUML. The built-in `!theme`s (carbon-gray etc.) only recolour ELEMENTS —
// the canvas stays WHITE, so black text on a white box reads terribly on a dark app. Real "readable dark" needs
// `skinparam` (the optionfirst deferred): a dark canvas + light text/lines across the common diagram
...
// #342inject the dark `skinparam` block into a PlantUML source, UNLESS the author already set their own
