...
export interface ConvergencePlanItem {
  ...
  keep: string
  remove: string[]
  // #536 null when the keeper is the principal's MANAGER standing rather than a row. That is the
  ...
  // #536 measured: a commenter tuple beside an aaa-role row). Swept by the leftover pass; listed
...
export async function planConvergence(
  ...
  // #536 who still holds `manage` on this space. A manager wearing a weaker role too is the last
