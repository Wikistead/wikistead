...
const LEDGER: Record<string, { kind: 'debt' | 'bounded' | 'internal'; why: string }> = {
  ...
  'admin-connections.ts:/admin/connections': { kind: 'bounded', why: '#623 (ruling 3): capped by MAX_OIDC_CONNECTIONS (20) at issue — the 21st POST answers 409 connection_limit_reached and the screen disables the add button at the cap the server reports. Pinned in connection-cap-623.' },
