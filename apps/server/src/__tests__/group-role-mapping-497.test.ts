...
//      assignment (theunassign). The assignment carries origin='mapping'.
...
afterAll(async () => {
  ...
  await admin`DELETE FROM role_assignments WHERE tenant_id = ${tenant.id} AND role_id IN (SELECT id FROM roles WHERE tenant_id = ${tenant.id} AND name LIKE 'grm497%')`
...
describe('group → role mappings (#497 / ADR-183)', () => {
  it('anti-test 1 (end-to-end): a mapping grants the role to a synced group member LIVE; delete reverts it', async () => {
    ...
    // Delete the mapping → theunassign reverts the leaves and drops both rows.
