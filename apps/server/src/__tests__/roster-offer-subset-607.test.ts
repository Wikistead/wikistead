...
beforeAll(async () => {
  ...
  // The owner also holds a plain `view` row — the exact two-row shape that produced the defect (// measured it on `dev-user` in demo_space). Written as tuples rather than through the grant route,
...
describe('#607 the roster offers no operation the server would refuse', () => {
  ...
  it('the roster verb changes ordinary principals and not admin-class ones — both, or the pin is empty', async () => {
    ...
    // above holds vacuously under either, which is exactly why asked for this premise.
    const roster = await listAllSpaceAccess(fgaClient, db, { spaceId, tenantId: T, userId: AM })
    expect(roster.length).toBeGreaterThan(2)
