...
describe('#828 the drain explains the address once', () => {
  it('three unaddressable messages produce one explanation, and it names the step', async () => {
    ...
    // `handled` below counts what it claimed. A row another test left behind (measured: 4 for 3 when
    // the full server suite runs together, green in isolation) is not this pin's subject — consume
    // whatever is pending first, so the count and the once-per-drain line are judged on these three
    // alone.
