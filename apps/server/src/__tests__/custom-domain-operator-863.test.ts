...
describe('#863 / ADR-258: local-admin --domain', () => {
  ...
  it('refuses a host another workspace already holds, naming the holder — including across a promoted schema', async () => {
    ...
    // registering `host` would still 23505 when the MIRROR row is written even with the all-tenant
    // walk deleted — a constraint answering "for free" rather than the walk this pin claims to guard.
    // The holder
