...
import { deleteAllFactors } from '../auth/second-factors.js' // #644the administrator reset (ADR-219 §4)
...
export async function membersPlugin(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { limit?: string; cursor?: string; q?: string } }>('/members', async (req, reply) => {
    ...
    >`SELECT m.sub, m.email, m.display_name, m.picture_url, m.role, m.groups, m.created_at,
             ...
             -- #644existence only, the same shape as has_password one line up — nothing about
  ...
  /**
   * Clear a member's second factors so they can enrol again (#644ruling 2 / ADR-219 §4).
   *
