...
export async function revokeApiKeyAsAdmin(
  ...
  return db.tx(async (tx) => {
    ...
    // #495 Q3 (ruled 2026-07-24): the api_key.revoked webhook carries the affected member's identity —
    // matching the GitHub-sender / Okta-actor norm for audit-class events. Enqueued in the SAME tx as
    // the revoke (the #228 webhook_outbox is transactional), so a commit-then-crash still delivers and
    // a rolled-back revoke never emits a webhook.
    //
    // ⚠️ #862 / ADR-108 addendum (ruled 2026-08-22): the id travels, the RESOLVED DISPLAY NAME does
    // not. What #495 Q3 put through Review was `ownerId`; `ownerName` came along beside it and was
    // never reviewed on its own — and this call site enqueues directly, so the bridge's verdict table
    // never saw it either. The one event already egressing was the one nobody had examined. A
    // consumer that wants the name asks the admin API for it, with its own authorization.
    await enqueueWebhookOutbox(tx, {
      tenantId: tenant.id,
      eventType: 'api_key.revoked',
      payload: { keyId: args.id, actorId: args.actorSub, ownerId: row.owner_user_id },
    })
