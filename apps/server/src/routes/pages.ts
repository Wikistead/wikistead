...
export async function updatePage(
  ...
  if (!canEdit) throw Object.assign(new Error('forbidden'), { statusCode: 403 })
  // #364a space HOME's title is derived from the space name and locked — the server is the
...
export async function toggleTask(
  ...
  // #361read → guard → fold now run inside ONE transaction under a per-page advisory lock.
  ...
  await db.tx(async (tx) => {
    ...
    // #361the two 409s carry DISTINCT static `code`s (the field Fastify's default error
...
export async function setPagePrivate(
  ...
  const oids = await db.tx(async (tx) => {
    ...
    // #511IDEMPOTENT. OpenFGA fails the whole batch when a tuple already exists, so privatising an
    ...
    try {
      ...
      // "reported as a permission skip" lie #511removed.
...
export async function getTitleDictionary(
  ...
  // `budgetMs` bounds the CONFIRM's checker occupancy (default 2000ms).'s slow mode, measured on
...
export async function pagesPlugin(app: FastifyInstance) {
  ...
  app.post<{ Params: { spaceId: string }; Body: { title?: string; parentId?: string | null; fromPageId?: string | null; templateId?: string | null } }>(
    '/spaces/:spaceId/pages', { config: { guest: 'edit' } }, async (req, reply) => {
      // #667every field here is optional, so a bodyless POST could have meant "an untitled
  ...
  app.patch<{ Params: { pageId: string }; Body: { parentId?: string | null; afterId?: string | null; spaceId?: string | null } }>(
    '/pages/:pageId/move', async (req) => {
      // #667every field defaults to null, so treating a missing body as `{}` would read as
  ...
  app.patch<{ Params: { pageId: string }; Body: { title: string } }>(
    '/pages/:pageId', { config: { guest: 'edit' } }, async (req) => {
      const body = requireBody(req.body) // #667the reported crash — `req.body.title` on undefined
  ...
  app.post<{ Params: { pageId: string }; Body: { source?: string; theme?: string } }>('/pages/:pageId/plantuml/render', { config: { guest: 'view' } }, async (req, reply) => {
    ...
    // #525distinguish the failure modes so the client can match mermaid — an INVALID diagram
