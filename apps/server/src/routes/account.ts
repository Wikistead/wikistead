...
// day = 'YYYY-MM-DD' in the caller's tz. #483 the count is split by kind (edits = revisions,
...
export async function getMyActivity(db: TenantDb, args: { subject: string; tz?: string }): Promise<{ tz: string; days: ActivityDay[] }> {
  ...
  // #483 the UNION ALL carries its kind through, so one pass yields the per-kind split the
