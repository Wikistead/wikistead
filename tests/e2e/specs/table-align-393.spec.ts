...
test("#393 addendum3 a rendered pipe table shows the hover align segment and promotes on center", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
test("#393 addendum3 the whole-table align segment shows while the edit island is open", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
