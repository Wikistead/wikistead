...
// createScratchPage for the full reasoning). Node's own fetch is not subject to browser CORS at all.
...
test("guest VIEW link shows the published version, never the live draft", async ({ browser }: { browser: Browser }) => {
  ...
  const pageId = await newPage("guest view page");
...
test("guest VIEW link renders a published image (guest image resolution)", async ({ browser }: { browser: Browser }) => {
  const member = await (await browser.newContext()).newPage();
  await openDemo(member);
  const pageId = await newPage("guest image page");
  await member.goto(`/p/${pageId}`);
  await member.waitForSelector("[data-pane=preview] .cm-content");
  await sleep(300);
  ...
  const list = (await (await fetch(`${API}/spaces/demo_space/pages/${pageId}/attachments`, { headers: { Authorization: "Bearer dev-token" } })).json()) as { filename: string; id: string }[];
  const attId = list.find((a) => a.filename === "g.png")!.id;
...
test("guest EDIT link can publish", async ({ browser }: { browser: Browser }) => {
  const member = await (await browser.newContext()).newPage();
  await openDemo(member);
  const pageId = await newPage("guest edit publish page");
  ...
  const publishedRes = await fetch(`${API}/pages/${pageId}/published`, { headers: { Authorization: "Bearer dev-token" } });
  const publishedMd = ((await publishedRes.json()) as { publishedMd: string | null }).publishedMd;
...
test("#917: an EDIT-link guest sees the unpublished badge while a draft diverges, and it clears on publish", async ({ browser }: { browser: Browser }) => {
  const member = await (await browser.newContext()).newPage();
  await openDemo(member);
  const pageId = await newPage("guest unpublished badge page");
...
test("#917: a VIEW-link guest never sees the unpublished badge (member-only surface)", async ({ browser }: { browser: Browser }) => {
  const member = await (await browser.newContext()).newPage();
  await openDemo(member);
  const pageId = await newPage("guest view no badge page");
  await member.goto(`/p/${pageId}`);
  await member.waitForSelector("[data-pane=preview] .cm-content");
  await sleep(300);
  await enterEdit(member);
