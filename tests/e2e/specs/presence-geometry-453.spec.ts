...
for (const kind of KINDS) {
  test(`#453a peer's box hugs the same rect as the local ring — ${kind.name}`, async ({ browser }) => {
    const ctxA = await browser.newContext();
    const ctxB = await browser.newContext();
    const A = await ctxA.newPage();
    const B = await ctxB.newPage();
...
test("#453a details block wears the shared atom-box marker (its geometry cannot be driven here)", async ({ browser }) => {
