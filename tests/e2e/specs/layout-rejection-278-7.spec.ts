...
//     on the rebuilt widget (supersedes thekeep-editing pin; the commit precedes mousedown side
...
test("#278-11the raw pill's signature is IDENTICAL at top level and inside an island", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
