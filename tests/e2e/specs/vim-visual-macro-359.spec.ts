...
// #359the GROWTH transitions. The two earlier pins start from a state that is already
...
test("#359a visual selection started INSIDE revealed source survives growing OUT of the block", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
test("#359a mouse drag started INSIDE revealed source survives growing OUT of the block", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
//
// Thepin above could not see this: it reads the head LINE, which steps correctly whether or
// not a selection exists. This one reads the selection.
test("#359a visual selection started ABOVE an adjacent cluster grows from the first press", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
