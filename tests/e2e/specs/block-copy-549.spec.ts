...
// ---------------------------------------------------------------------------------------------
// #549 reviewthe pins below use the MOUSE gesture — click the rendered block, then
...
for (const [tag, source, locate] of [
  ...
  test(`#549MOUSE click on a ${tag} → Ctrl+C takes the whole source`, async ({ browser }) => {
...
test("#549the bare ```mermaid fragment can NEVER be what a block click copies", async ({ browser }) => {
...
test("#549KEYBOARD onto the marker line (arrow into the block) → Ctrl+C still takes the whole source", async ({ browser }) => {
...
test("#549tabs PANEL click enters the slot (#278) — Ctrl+C takes the INNERMOST block the ring shows", async ({ browser }) => {
...
test("#549tabs CHROME click (no slot entry) — Ctrl+C takes the whole container", async ({ browser }) => {
...
test("#549normal editing is untouched — Ctrl+C on a revealed CONTENT line copies that line only", async ({ browser }) => {
