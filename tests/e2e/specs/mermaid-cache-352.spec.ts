...
// #352the review found the TOP diagram missing the cache EVERY round-trip (id 4→7→8) — a paint
...
test("#352the top mermaid caches across TWO round-trips (torture body, no per-trip re-render)", async ({ browser }) => {
  const page = await (await browser.newContext({ viewport: { width: 1310, height: 940 } })).newPage();
  await openScratch(page, "mermaid-torture");
  await enterEdit(page);
  await page.click("[data-pane=preview] .cm-content");
  // A known diagram at the very TOP (thepermanent-miss was the top diagram), then the real torture body
  ...
  expect(afterRT2).toBe(afterRT1); // stable id ⇒ no re-render on the 2nd round-trip (thepermanent-miss gone)
});
