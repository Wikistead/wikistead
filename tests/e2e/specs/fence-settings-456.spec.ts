...
// #456 item 2: a keyboard path (Mod-Alt-Enter) and a hover ✎ open the same panel — right-click was
// the only way in. item 3: the panel can be closed with the × button and with Escape.
test("#456 the settings open from the keyboard and from a hover ✎, and close via × and Escape", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
