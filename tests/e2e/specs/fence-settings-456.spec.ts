...
// #456a BARE ``` fence (no info string) silenced all three affordances — the old resolver's
...
test("#456a bare ``` fence opens the settings from all three affordances", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
test("#456a ~~~ fence resolves too", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
