...
test("#457 the GUEST body draws the same opaque loading overlay, then the content", async ({ browser }) => {
  ...
  // opaque — the lesson applies to the guest surface too (no skeleton/body overlap window)
  const bg = await ph.evaluate((el) => getComputedStyle(el).backgroundColor);
  const m = bg.match(/rgba?\(([^)]+)\)/);
...
test("#457 a guest page with no published body says empty — never an eternal skeleton", async ({ browser }) => {
  const member = await (await browser.newContext()).newPage();
...
test("#457 the GUEST sidebar shows a tree skeleton while the tree loads — never the empty wording", async ({ browser }) => {
  const member = await (await browser.newContext()).newPage();
...
test("#457 the right panels (comments / history / attachments) skeleton while their lists load", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
test("#457 the search result list skeletons while a slow query runs", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
  await openDemo(page);
...
// The PUBLIC reader (ShellLoading, #364 wired it; this pins it — nothing pinned it before, so a
// regression to the old blank div would go unseen). Same make-public idiom as public-page.spec.ts.
test("#457 the public reader skeletons while the page resolves", async ({ browser }) => {
