...
// #278B: the shipped island-vim-`o` test covered COLUMNS only; the review bounce suspected a
...
test("#278B: TAB-panel island last-line `o` stays inside (parity with columns)", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
