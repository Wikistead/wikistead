...
// #653four things the user found on the device, none of which any green test noticed.
//
...
async function enrol(page: import("@playwright/test").Page, label: string): Promise<string> {
  ...
  for (let attempt = 0; ; attempt++) {
    await page.getByTestId("factor-add").click(); // #653unnamed; named after it exists
    try {
  ...
  // #653the row arrives unnamed, and this spec follows it by name afterwards — so it is named
 ...
 * being written, and #653recorded the same trap one round earlier.
 */
