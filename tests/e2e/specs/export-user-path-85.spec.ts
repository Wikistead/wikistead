...
test("#85 the downloaded file, opened with the app closed, IS the document", async ({ page, browser }) => {
  ...
  // #579 whole name, not a prefix — "Heading one" would also match a "Heading one and a half"
  ...
  //
  // Break-check established (#633 asked; a first attempt could not and said so): forcing
  ...
  //
  // #633 this block was deleted in the same commit that re-aimed the width check above, and that
...
test("#85 an embedded page travels with the file, not a 'loading' that never resolves", async ({ page }) => {
  test.setTimeout(240_000);
  await page.addInitScript(() => { try { localStorage.setItem("wks.lang", "ja"); } catch { /* private mode */ } });
