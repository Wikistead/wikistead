...
// #528red-check, pinned: across a sweep of pointer positions and entry states, the set of VISIBLE
...
test("#528no two visible affordances intersect — swept across hover positions and entry states", async ({ browser }) => {
  ...
  // entry state: into the nested slot (island), Esc back out — the statesmeasured around
...
test("#528a still pointer sees ONE stable visible-affordance set (no flicker)", async ({ browser }) => {
  const page = await setup(browser);
  const wrap = page.locator("[data-pane=preview] .cm-lp-macro-wrap").first();
  const bb = (await wrap.boundingBox())!;
