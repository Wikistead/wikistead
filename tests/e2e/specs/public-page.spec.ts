...
test("#319 public reader has the reading column AND no read-only macro edit affordances", async ({ browser }) => {
  const authed = await (await browser.newContext()).newPage();
...
// #319 the public reader is now the SAME mountPublishedView engine as the editor, but a leftover
...
test("#319 a table renders at the SAME (content) width in the editor and the public reader", async ({ browser }) => {
...
test("#430: the standalone public page has the minimal header — brand + powered-by + theme, NO member chrome", async ({ browser }) => {
  ...
  // #430 the brand is the shared two-slot TenantBrand now (the same one the member header and
