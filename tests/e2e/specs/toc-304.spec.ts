...
// #345Issue B: when the page opens with a top INTRO (no heading at the very top) but headings on screen
...
test("#345Issue B: a heading below a top intro still lights (active falls back to the topmost visible)", async ({ browser }) => {
  const page = await (await browser.newContext({ viewport: { width: 1600, height: 720 } })).newPage();
...
// #345Issue A: the narrow-screen OVERLAY TOC now also gets the light (data-visible) layer — it was
// rail-only (TocChrome passed visibleFroms only to the rail). Real Chromium, narrow viewport, scroll to reveal.
test("#345Issue A: the narrow overlay TOC carries the two-layer highlight", async ({ browser }) => {
...
// #345Issue 1: on a long TOC the rail auto-follows the highlight so the active item never scrolls out of
// the rail as the reader nears the bottom (the old single-active centring dropped it off).
test("#345the rail follows the highlight to the bottom — the active item stays visible", async ({ browser }) => {
...
// #345Issue 3: hovering the narrow-screen overlay TOC holds it open (it no longer fades from under the
// reader after the ~1.2s scroll timeout).
test("#345hovering the overlay TOC holds it open past the fade timeout", async ({ browser }) => {
  const page = await (await browser.newContext({ viewport: { width: 1600, height: 720 } })).newPage();
