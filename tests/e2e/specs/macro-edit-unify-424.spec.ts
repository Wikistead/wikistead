...
test("#424details + warning callout edit buttons sit at the unified top-left, inside the block", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
  ...
  for (const mode of ["live", "wysiwyg"] as const) {
    ...
    // Visible ON HOVER inside the viewport (thesymptom was "no hint appears on warning").
