...
// #431the SAME user must show the SAME initials on every surface. In god-mode the header /
...
test("#431god-mode header initials come from the canonical display name, not the sub", async ({ browser }) => {
...
// #431the surface the earlier pass never pinned. The header was fixed to read the canonical
...
test("#431the created/updated meta avatar matches the header for the same (un-customized) user", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
