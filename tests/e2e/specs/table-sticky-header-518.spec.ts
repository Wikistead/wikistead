...
// scroll sideways). The device trace found the attempt was false-green: it exercised a GFM
...
test("#518: a tall+wide :::table pins its header (top not auto) and scrolls INSIDE its own box", async ({ browser }) => {
  ...
  expect(rep.top, "sticky top is a real offset, NOT auto (the bug)").not.toBe("auto");
