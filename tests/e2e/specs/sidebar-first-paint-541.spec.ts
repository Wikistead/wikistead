...
test("#541: a realistic 197-page space's sidebar arrives with the body — on every back-to-back cold open", async ({ browser }) => {
  ...
  // Sidebar arrival = the first TREE ROW paints ('s method — the text-length probe never fires
  ...
  for (let run = 1; run <= RUNS; run++) {
    ...
    // dictionary must not starve this one (theslow mode).
  ...
  // ──'s PRIMARY pin: proportionality, not wall clocks. A 5-page space and the big space are
  ...
  // must stay bounded). Pre-progressive this measured ~6× on dev; ≤2 is's acceptance.
