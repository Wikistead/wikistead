...
async function checkSurface(page: Page) {
  ...
  // #389 the ring paints its own dot as a background gradient (one paint box, so the dot
...
// only checked `transform` passed while the dot was invisible). Since the ring paints the dot
