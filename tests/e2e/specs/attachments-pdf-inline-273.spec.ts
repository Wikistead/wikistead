...
// #273 hover feedback must name the region it belongs to. split the inline-PDF card into
...
test("#273 hovering the header leaves the preview alone; hovering the preview dims it", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
// #273 non-regression: a DOWNLOAD card (no preview frame) is one full-surface click target, so
// hovering it anywhere must still light its ⤓ — the header-scoped rule covers it because the card row
// IS the whole card there.
test("#273 a download card still lights its ⤓ on hover (single-region card)", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
