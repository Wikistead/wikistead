...
// #395the policy hint must (A) live on a surface EXISTING users can reach — Settings →
...
test("#395the atom-policy hint is reachable in account settings and matches the implementation", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
test("#395selecting an atom (ring) reveals the Ctrl+Enter entry pill", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
test("#395clicking a rendered callout never auto-opens the edit form (Live + WYSIWYG)", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
