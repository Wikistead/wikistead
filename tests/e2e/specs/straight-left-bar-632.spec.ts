...
// green — the exact failure names (demanding a discovery pin while doing an enumerated sweep).
...
test("#632: the bar is still there, still 3px, still the type's colour", async ({ page }) => {
  ...
  const strips = await page.evaluate(() =>
    [...document.querySelectorAll<HTMLElement>("[data-pane=preview] .cm-lp-callout")].slice(0, 4).map((el) => {
      const cs = getComputedStyle(el);
      // #632 the bar is a background band now, so it is measured off the gradient — the first
