...
// #416the Permissions dialog must NEVER outgrow the viewport — bounded max-h flex column,
...
test("#416the dialog stays inside the viewport with 12 grants; Close reachable; body scrolls", async ({ page }) => {
  await openDemo(page);
...
// #460Radix auto-focused the first tabbable — the active TabsTrigger — on open, and
...
test("#460a mouse open paints no focus ring on the Access tab; keyboard tabbing still works", async ({ page }) => {
  await openDemo(page);
  await page.click("[data-testid=page-overflow-trigger]");
  await page.click("[data-testid=permissions-open]");
  await expect(page.locator("[data-testid=permissions-dialog]")).toBeVisible();
