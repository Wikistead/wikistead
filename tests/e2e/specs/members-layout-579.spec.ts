...
test("#579the two operations stay two, and the row keeps one of each control", async ({ page }) => {
...
test("#579a tier option still says what it confers, from this tenant's own defaults", async ({ page }) => {
  test.setTimeout(120_000);
  await page.setViewportSize(VIEWPORT);
  await openDemo(page);
  await page.goto("/admin/members");
