...
// ring + pill — and the pill sat across the (container's) ring line.'s ruling is "the focused
...
// exclusivity (slotEdit counts as nested-active) closes it; this spec pins's two measurements so
...
test("#528a nested Excalidraw selection shows ONE chrome set — the container's yields", async ({ browser }) => {
...
test("#528the same exclusivity holds for a nested mermaid (the reveal-type atom)", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
  await author(page);
