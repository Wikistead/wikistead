...
// #455alignment applies to the RENDERED diagram, never to the "still empty" hint. toDOM
...
test("#455aligning an EMPTY macro leaves its placeholder full width", async ({ browser }) => {
  const page: Page = await (await browser.newContext()).newPage();
  ...
  // An align-only change goes through updateDOM (#255keeps the rendered SVG rather than
