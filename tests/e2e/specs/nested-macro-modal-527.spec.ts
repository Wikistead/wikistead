...
test("#527: a scene edited in a NESTED macro's modal survives the save", async ({ browser }) => {
  ...
  // One click enters the slot island (the #278 ruling); inside it the nested macro behaves
...
test("#527 after a modal edit in one container, a click still enters the other", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
test("#527 after a keyboard edit in one container, a click still enters the other", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
