...
test("#445 a refused creation says why, and keeps the typed name", async ({ page }) => {
  await openDemo(page);
...
test("#445 the entry point is hidden when the member may not create spaces", async ({ page }) => {
...
test("#445 a STALE allow-flag still produces the error (hiding is not the feedback)", async ({ page }) => {
...
test("#445 creation still works when it is allowed (no regression)", async ({ page }) => {
  await openDemo(page);
  await openNewSpaceDialog(page);
