...
// #361, MIGRATED by theowner ruling ("a burst 409 is acceptable if that buys speed").
...
test("#361a fast burst across DIFFERENT boxes folds every flip — no publish-first block", async ({ browser }) => {
...
test("#361a fast SAME-box triple click folds every flip in order — no publish-first block", async ({ browser }) => {
...
test("#361P0: with the toggle stalled 2s, the click frame flips the box AND all three rings", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
...
// #361a BURST no longer waits on the previous round-trip, and the server folds every pending
...
test("#361a 4-click burst lands every flip; no error toast, no rollback", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
