...
// #361 the DOUBLE-CLICK path. A rapid second click makes the two optimistic draft flips cancel
...
test("#361 a rapid double-click (ON-start) shows no extra blink and leaves no dirty residue", async ({ browser }) => {
  await doubleClickPin(browser, 0, false);
});

test("#361 a rapid double-click (OFF-start) shows no extra blink and leaves no dirty residue", async ({ browser }) => {
  await doubleClickPin(browser, 1, true);
});

// #361 the FAST-CLICK flicker. preventDefault on mousedown does NOT stop a native checkbox's
...
test("#361 a real fast click never shows the reverted state while the toggle is in flight", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
