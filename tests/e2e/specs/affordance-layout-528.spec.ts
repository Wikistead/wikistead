...
// its raw-entry pill. That is the collision this ticket exists for (#528 measured it at 8px). The
// nested fixture above no longer produces it, and for the right reason: since #528 only the focused
...
test("#528 only the FOCUSED block offers an affordance (asserted by ownership, not by count)", async ({ page }) => {
  ...
  // #528 rewrote this check. The previous version decided which block was "focused" by looking at
  ...
  // …and the container it lives in is not the one wearing the focus mark (#528 measured it was)
