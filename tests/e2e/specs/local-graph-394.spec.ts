...
// #394the animation pin the first pass lacked (false-green: "canvas exists" can't catch a
...
test("#394the layout ANIMATES — display coordinates move between frames (mini + modal)", async ({ browser }) => {
  const page = await (await browser.newContext()).newPage();
