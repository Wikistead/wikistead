...
| **Macro level cap** (`macroLevelCap`) | The highest MacroTier layer a tenant may persist (#93 / ADR-073). The host auto-demotes to it at persist time (server fortress); directive = no cap. | directive | persist-time normalization (auto-demote to min(lowest-representable, cap)) | content normalizes to the cap layer (non-destructive, round-trips where representable) |
