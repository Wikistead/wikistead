...
| **SCIM provisioning** (`scim`) | EE | SCIM directory provisioning — tokens + endpoints (EE). | Not on self-host (EE-only) | SCIM token issuance + /scim/v2 endpoints | gated; EE-only |
...
| **SAML SSO** (`samlSso`) | EE | Workspace SAML single sign-on — configuration + login (EE). | Not on self-host (EE-only) | workspace SAML config + /auth/saml | gated; EE-only (SAML sign-in is available only while the plan includes it) |
...
| **Compliance audit log** (`auditLog`) | EE | Durable, tamper-evident audit log of permission and compliance operations (EE). | Not on self-host (EE-only) | when an auditable operation is recorded (skipped when disabled) | gated; EE-only (no audit ledger written for CE/free) |
| **Access Transparency** (`accessTransparency`) | EE | Discloses operator break-glass accesses to the workspace — a tamper-evident, per-workspace view of the sealed operator log (EE). | Not on self-host (EE-only) | the admin Transparency screen and its API (GET /admin/transparency + /verify) | gated; rows retained and hidden |
| **Page analytics (who-viewed)** (`analytics`) | EE | Per-viewer page analytics: members named in a roster, guests/anonymous aggregated (EE). | Not on self-host (EE-only) | when views are collected, and at the dashboard (collection itself is gated — no history accrues for CE/free plans) | gated; collection stops, retained rows follow the retention/erasure policy |
