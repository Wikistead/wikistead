...
export const LEVER_CATALOG: Record<keyof Entitlements, LeverDoc> = {
  ...
  customDomain: {
    title: 'Custom domain',
    summary: 'A tenant custom domain such as docs.acme.com.',
    unit: 'boolean',
    enforcedAt: 'custom-domain add/verify',
    downgrade: 'three-point revoke on loss — row + host→tenant map + cert',
  },
  scim: {
    edition: 'ee',
  ...
  aiFeatures: {
    title: 'AI assists',
    summary: 'AI features (summarize / ask-KB / etc.). Also requires a registered AIProvider (BYOK) — this is the PLAN lever, the provider is the deployment switch.',
    unit: 'boolean',
  ...
  samlSso: {
    edition: 'ee',
    title: 'SAML SSO',
    summary: 'Tenant SAML single sign-on — config + login (EE).',
    unit: 'boolean',
  ...
  managedEmail: {
    ...
    summary: 'Notification email rides the managed provider driver instead of self-hosted SMTP.',
    unit: 'boolean',
  ...
  auditLog: {
    ...
    summary: 'Durable, hash-chained audit ledger of authz/compliance operations (EE).',
    unit: 'boolean',
  ...
  accessTransparency: {
    ...
    summary: 'Tenant-facing disclosure of operator break-glass accesses — a per-tenant, hash-chained projection of the sealed operator ledger (EE).',
    unit: 'boolean',
  ...
  userMacros: {
    ...
    summary: 'Whether the tenant may run non-first-party macros. Requires this AND a tenant-admin allowlist; macros never self-authorize. First-party always allowed.',
    unit: 'boolean',
  ...
  mcpWrite: {
    ...
    summary: 'Whether the tenant may use MCP WRITE tools (create/edit/publish via the connector). MCP read tools are all-plans; only write is gated (write = Cloud/EE). OpenFGA still gates each resource.',
    unit: 'boolean',
  ...
  spaceEditLink: {
    ...
    summary: 'Whether the tenant may ISSUE a space-wide EDIT share-link (anonymous collaborative wiki). Cloud = paid tiers only; self-host unlimited. OpenFGA still gates every page the link reaches.',
    unit: 'boolean',
  ...
  customRoles: {
    title: 'Custom roles',
    summary: 'Whether the tenant may DEFINE and ASSIGN custom roles — named bundles of atomic capabilities (view/comment/edit/publish/delete/share/settings/moderate). Built-in roles are free on every plan. OpenFGA stays the single authz truth: a role only chooses which fixed-relation tuples to write.',
    unit: 'boolean',
  ...
  apiRateLimit: {
    ...
    summary: 'Authenticated API-key request rate per window — perKey (per-key fairness) and perTenant (all-keys ceiling), evaluated AND (the stricter trips first → 429).',
    unit: 'rate',
